...
export const BankrollStats = () => {
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      ...
      const {
        data: bets
      } = await supabase.from("bets").select("*").eq("user_id", user.id);
      if (!bets) {
        setStats({
      ...
      const percentChange = initialBankroll > 0 ? (currentBankroll - initialBankroll) / initialBankroll * 100 : 0;
    ...
    const channel = supabase.channel('bets-changes').on('postgres_changes', {
      event: '*',
      schema: 'public',
      table: 'bets',
  ...
  return <div className="flex items-center gap-6 px-4 py-2 bg-muted/30 border-b border-border">
